...
local function keyToString(key)
  return keySymbols[key:lower()] or key:upper()
end

local function hotkeyToString(hotkey)
  local keys = {}

  for _, v in ipairs(hotkey[1]) do
    table.insert(keys, keyToString(v))
  end
...
local function createMenuFn(menu)
  for i in ipairs(menu) do
    ...
    if menu[i].fn and menu[i].hotkey then
      ...
      menu[i].title = menu[i].title .. " [" .. hotkeyToString(menu[i].hotkey) .. "]"
    end
  end
end
