...
function types.timestamp:__tostring()
    return string.format('%04d-%02d-%02dT%02d:%02d:%02d.%03d%s', self.year,
                         self.month, self.day, self.hour, self.minute,
                         self.second, self.fraction, self:gettz())
end
...
function Parser:parsestring(line, stopper)
    ...
    while #line > 0 do
        ...
        if sfind(stopper, c, 1, true) then
            break
        elseif c == ':' and
            (ssub(line, 2, 2) == ' ' or ssub(line, 2, 2) == '\n' or #line == 1) then
            break
...
return {version = 0.1, parse = parse}
